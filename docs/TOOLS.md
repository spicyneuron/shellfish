...
A manifest may override `initial_user_text` and `permission_user_text` before execution and `user_text` and `model_text` for the result. Supplied fields merge over defaults that show the tool name and input on one line, then return stdout and stderr without an exit code. Empty rendered text is omitted. `render.preview_lines` may be `"full"` or a non-negative line count; when omitted, the TUI uses its default preview limit. Templates use one-pass `${...}` substitution with `name`, `input`, `input.FIELD`, `output.stdout`, `output.stderr`, and `output.exit_code`. Output variables are available only to result templates. See the manifests under [`share/default/tools/`](../share/default/tools/) for examples.
