...
Backend, tool, and hook manifests declare which component-specific environment variables their scripts receive. Values are resolved from exported variables and then `.env`; missing values remain unset. Names and the `.env` location are frozen in the session, while values remain external and are resolved for each invocation.

Backends and hooks inherit the ordinary process environment, but Shellfish withholds all configured component variables and restores only those selected for that script. Tools instead start with a clean environment and receive only their selected values and fixed Shellfish tool context.
