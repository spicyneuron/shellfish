...
test("reports session compaction without replaying the transcript", async () => {
  ...
  assert.equal(findTag(notes[0], "h2")[0].textContent, "ℹSession compacted");
  assert.equal(findTag(notes[1], "h2")[0].textContent, "ℹCompaction skipped");
  assert.equal(findTag(notes[1], "pre")[0].textContent, "summary unavailable");
  assert.equal(page.reloads, 0);
});
