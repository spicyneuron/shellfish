...
cat >"$compact_backend" <<ZSH
#!/usr/bin/env zsh
request=\$(cat)
print -r -- "\$request" >>"\$COMPACT_REQUESTS"
print -r -- "\$request" | exec "$SF_TEST_BACKEND"
ZSH
chmod +x "$compact_backend"
cat >"$compact_hook" <<'ZSH'
#!/usr/bin/env zsh
cat
print -n context
ZSH
...
sf_test_turn first "$compact_session" >/dev/null
sf_test_turn middle "$compact_session" >/dev/null
sf_test_turn latest "$compact_session" >/dev/null
...
: >"$compact_requests"
compact_stream=$(sf_test_turn under "$below_session")
if print -r -- "$compact_stream" | jq -eR 'fromjson | select(.type == "_session_compaction")' \
    >/dev/null; then
  fail 'session compacted below the threshold'
fi
: >"$compact_requests"
compact_stream=$(sf_test_turn continue "$compact_session")
compact_path=$(print -r -- "$compact_stream" | jq -Rr '
  fromjson | select(.type == "_session_compaction") | .session
')
[[ $compact_path == "$tmp/compact-source_compact.jsonl" ]]
...
' >/dev/null
jq -e -s '
  [.[] | select(.role == "user") | .content[0].text] == ["first","middle","latest"] and
  ([.[] | select(.type == "context") | .content] | index("continuecontext") | not)
' "$compact_session" >/dev/null
jq -L "$ROOT/lib" -e -s '
  include "runtime/schema";
...
jq -e -s '
  length == 2 and
  .[0].messages[-1].role == "user" and
  (.[0].messages[-1].content[0].text | startswith("Create a concise continuation summary")) and
  (.[1].messages[-1].content[0].text | contains("continuecontext") and endswith("continue"))
' "$compact_requests" >/dev/null
SF_TEST_RUNTIME=$base_runtime
