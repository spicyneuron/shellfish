...
sf_chat_event assistant_delta 'part '
sf_chat_event assistant_delta done
...
if sf_chat_set_frontier 1 5; then
...
sf_chat_reset
sf_chat_event assistant_reasoning_delta thought
sf_chat_event reasoning_tokens 7
...
sf_chat_event tool_call call_2 read_file README.md '' plain
sf_chat_event tool_permission 'host access'
assert_equal 1 "$REPLY"
assert_equal permission "$SF_PRESENT_NODE_STATUS[5]"
assert_equal 'host access' "$SF_PRESENT_NODE_BODY[5]"
sf_chat_event tool_permission_clear
...
assert_equal closed "$SF_PRESENT_NODE_STATE[5]"
sf_chat_event tool_result call_2 0 contents
...
assert_equal 0 "${#SF_PRESENT_NODE_TYPE}"
sf_chat_section agent
assert_equal section "$SF_PRESENT_NODE_TYPE[1]"

sf_chat_reset
sf_chat_event user $'unsafe\e[31m'
sf_chat_event assistant_delta $'reply\rtext'
...
sf_chat_reset
sf_chat_section agent
sf_chat_add tool_result agent shell result
sf_chat_event backend_request_start
...
if sf_chat_add notice '' error failed; then
  fail 'accepted a node after an open tail'
fi

sf_chat_reset
sf_chat_add notice '' working '' open
if sf_chat_event assistant_commit; then
  fail 'closed a non-assistant node on assistant commit'
fi

sf_test_tmp presentation
sf_chat_reload "$SF_TEST_SESSIONS/tool-paired.jsonl" || fail "$SF_PRESENT_ERROR"
...
if TERM=dumb sf_chat_theme_config "$theme_config"; then
  assert_equal 0 "${#SF_PRESENT_STYLE}"
else
...
sf_chat_background_mode() { (( ++probe_calls )); REPLY=light; }
...
SF_PRESENT_BACKGROUND=''
sf_chat_background_mode() { return 1 }
sf_chat_theme_config "$auto_config" || fail 'an unanswered probe should fall back to dark'
...
SF_PRESENT_HIGHLIGHT_SPANS=()
sf_chat_markdown_highlight '- not a list marker' 0 '' 1
assert_equal 0 "${#SF_PRESENT_HIGHLIGHT_SPANS}"
SF_PRESENT_HIGHLIGHT_SPANS=()
...
sf_chat_markdown_highlight '```' 0 $'```\tjs\t0' 1
...
sf_chat_markdown_highlight 'a **bold start** done'
assert_equal 0 "$SF_PRESENT_HIGHLIGHT_INLINE_OPEN"
sf_chat_markdown_highlight 'a `code start'
assert_equal 1 "$SF_PRESENT_HIGHLIGHT_INLINE_OPEN"
...
SF_PRESENT_HIGHLIGHT_SPANS=()
sf_chat_code_highlight '/* opened' js
assert_equal 1 "$SF_PRESENT_HIGHLIGHT_BLOCK_OPEN"
span_texts '/* opened'
assert_equal '/* opened' "$REPLY"
SF_PRESENT_HIGHLIGHT_SPANS=()
sf_chat_code_highlight 'still inside */ const x = 3;' js 0 1
...
sf_chat_markdown_highlight $'closed */\n' 0 $'```\tjs\t1' 0
assert_equal $'```\tjs\t0' "$REPLY"
...
sf_chat_reset
sf_chat_add message agent '' $'closed\nbody' open
sf_chat_highlight_update
...
functions[sf_chat_markdown_highlight]=$functions[sf_chat_markdown_original]
unfunction sf_chat_markdown_original

# Cache entries follow the nodes they describe when the front is pruned.
sf_chat_reset
sf_chat_add message agent '' first
sf_chat_add message agent '' second
sf_chat_highlight_update
...
sf_chat_reset
sf_chat_add message agent '' $'**abcdef**\n' open
sf_chat_highlight_update
sf_chat_highlight_prune 1 3
...
sf_chat_reset
sf_chat_add message agent '' "${(l:60::x:)""}" open
sf_chat_highlight_update
assert_equal 0 "$SF_PRESENT_NODE_FRONTIER[1]"
sf_chat_set_frontier 1 40
sf_chat_append 1 y
sf_chat_highlight_update || fail 'a later highlight retreated the frontier'
