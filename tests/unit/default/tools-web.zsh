...
jina_output=$(PATH="$tmp/jina-bin:$PATH" JINA_TEST_ARGS="$jina_args" \
  "$ROOT/share/tools/fetch_url/run" <<<'{"url":"https://example.com/docs?q=reader"}')
...
if PATH="$tmp/jina-bin:$PATH" JINA_TEST_ARGS="$jina_args" \
    "$ROOT/share/tools/fetch_url/run" <<<'{"url":"file:///etc/passwd"}' >/dev/null 2>&1; then
  fail 'fetch_url accepted a non-HTTP URL'
fi
if PATH="$tmp/jina-bin:$PATH" JINA_TEST_ARGS="$jina_args" \
    "$ROOT/share/tools/fetch_url/run" <<<'{"url":"https://example.com","extra":true}' >/dev/null 2>&1; then
  fail 'fetch_url accepted an unknown input field'
fi
if PATH="$tmp/jina-bin:$PATH" JINA_TEST_ARGS="$jina_args" JINA_TEST_STATUS=22 \
    "$ROOT/share/tools/fetch_url/run" <<<'{"url":"https://example.com"}' >/dev/null 2>&1; then
...
' "$ROOT/share/tools/fetch_url/fence.jsonc" >/dev/null
...
exa_output=$(PATH="$tmp/exa-bin:$PATH" EXA_TEST_ARGS="$exa_args" \
  "$ROOT/share/tools/search_web/run" \
...
if PATH="$tmp/exa-bin:$PATH" EXA_TEST_ARGS="$exa_args" EXA_TEST_RESPONSE="$exa_error" \
    "$ROOT/share/tools/search_web/run" <<<'{"query":"docs"}' >/dev/null 2>&1; then
...
jq -e '
  ...
  .network.allowLocalOutbound == false and
  .filesystem.defaultDenyRead == true
' "$ROOT/share/tools/search_web/fence.jsonc" >/dev/null
