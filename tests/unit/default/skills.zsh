...
sf_test_tmp skills
source "$ROOT/share/profiles/default/lib/skills.zsh"

tool="$ROOT/share/profiles/default/tools/skill/run"
project="$tmp/project"
config="$tmp/config"
home="$tmp/home"
...
HOME="$home" sf_skills_discover "$ROOT/share/profiles/default" "$config" "$project"
...
HOME="$home" sf_skills_discover "$ROOT/share/profiles/default" "$config" "$claude_project"
...
loaded=$(cd "$project" && print -rn -- '{"name":"skill-creator"}' | HOME="$home" \
  SHELLFISH_CONFIG_DIR="$config" zsh -f "$tool")
[[ $loaded == "Skill directory: $ROOT/share/profiles/default/skills/skill-creator"*$'\nname: skill-creator\n'* ]]
