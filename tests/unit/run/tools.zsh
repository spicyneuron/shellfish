...
SF_TEST_RUNTIME=$(jq -c --arg pre "$pre" --arg later "$later" --arg post "$post" '
  .harness.pre_tool_use=[
    {command:$pre,environment:["HOOK_DIR"]},
    {command:$later,environment:["HOOK_DIR"]}
  ] |
  .harness.post_tool_use=[{command:$post,environment:["HOOK_DIR"]}]
' <<<"$SF_TEST_RUNTIME")
...
SF_TEST_RUNTIME=$(jq -c --arg hook "$permission" '
  ...
  .harness.permission_request=[{command:$hook,environment:["PERMISSION_INPUT"]}]
' <<<"$SF_TEST_RUNTIME")
...
SF_TEST_RUNTIME=$(jq -c --arg hook "$post_fail" '
  ...
  .harness.post_tool_use=[{command:$hook,environment:[]}]
' <<<"$SF_TEST_RUNTIME")
