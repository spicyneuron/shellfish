...
jq -e '. == {turn_id:1,tool:{id:"call_1",name:"shell",
  input:{command:"printf headless",request_sandbox_bypass:true,
...
jq -e -s '([.[] | select(.type == "hook_result")] | length) == 0' \
...
#!/usr/bin/env zsh
print -rn -u3 -- '{"action":"deny","reason":"risk too high","state":[{"name":"permissions/1/call_1","value":{"content":null,"reason":"risk too high"}}]}'
exit 11
ZSH
...
print -r -- "$stream" | jq -eRn '
  ...
  ($events | map(select(.type == "tool_result"))[0] |
    .exit_code == 126 and .model_text == "risk too high\nexit 126")
' >/dev/null
...
print -r -- "$stream" | jq -eRn '
  ...
  ($events | map(select(.type == "_tool_permission_request")) | length) == 0 and
  ($events | map(select(.type == "tool_result"))[0] |
    .exit_code == 126 and .model_text == "sandbox bypass denied\nexit 126")
' >/dev/null
...
print -r -- "$stream" | jq -eRn '
  ...
  ($events | map(select(.type == "_tool_permission_request"))) ==
    [{type:"_tool_permission_request",id:"permission_1",reason:"Required by the test fixture",
      preview:"printf approved",
      tool:{id:"call_1",name:"shell",
  ...
  ($events | map(select(.type == "tool_result"))[0] |
    .exit_code == 0 and .model_text == "approved\nexit 0")
' >/dev/null
assert_equal "$frozen_tools" "$(jq -c '.tools' "$request_capture")"
...
print -r -- "$stream" | jq -eRn '
  ...
  ($events | map(select(.type == "_tool_permission_request")) | length) == 1 and
  ($events | map(select(.type == "tool_result"))[0] |
    .exit_code == 126 and .model_text == "tool call interrupted\nexit 126") and
...
print -r -- "$stream" | jq -eRn '
  ...
  ($events | map(select(.type == "tool_result"))[0] |
    .id == "call_1" and .exit_code == 126) and
